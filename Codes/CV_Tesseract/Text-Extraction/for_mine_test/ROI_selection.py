...
def detect_lines(image, title='default', rho = 1, theta = np.pi/180, threshold = 120, minLinLength = 90, maxLineGap = 100, display = False, write = False):
    ...
    # dst = cv.Canny(gray, 150, 255, None, 3)

    _, thresh = cv.threshold(gray, 150, 255, cv.THRESH_BINARY)

    edged = cv.Canny(thresh, threshold1=127, threshold2=255)

    linesP = cv.HoughLinesP(edged, 1, np.pi/180, threshold, minLinLength, maxLineGap)
    
    #linesP = cv.HoughLinesP(dst, 1 , np.pi / 180, 50, None, 290, 6)
    # linesP = cv.HoughLinesP(edged, rho , theta, threshold, None, minLinLength, maxLineGap)
